...
const PatientsList = () => {
  ...
  if (error) return (
    <Card className="p-6 bg-red-50 border-red-200">
      ...
      <Button onClick={refetch} variant="destructive" className="mt-2">Retry</Button>
    </Card>
  );
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button onClick={() => navigate('/patients/add')} className="bg-blue-600 hover:bg-blue-700">
          Add Patient
        </Button>
      </div>
          ...
          <Card key={patient.id} className="hover:shadow-lg transition-all duration-200">
            ...
            <CardContent className="space-y-2">
              ...
              <div className="flex gap-2 mt-4">
                ...
                <Button size="sm" variant="outline" onClick={() => navigate(`/patients/${patient.id}/edit`)}>
                  Edit
                </Button>
                <Button size="sm" variant="destructive" onClick={() => setDeleteModal({ open: true, patient })}>
                  Delete
                </Button>
              </div>
            </CardContent>
      ...
      {pagination.totalPages > 1 && (
        <div className="flex justify-center gap-2">
          ...
          <span className="flex items-center px-4">Page {currentPage + 1} of {pagination.totalPages}</span>
          <Button variant="outline" onClick={() => setCurrentPage(p => p + 1)} disabled={currentPage >= pagination.totalPages - 1}>
            Next
          </Button>
        </div>
      )}
