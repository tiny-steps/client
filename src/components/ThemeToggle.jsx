...
/**
 * A theme toggle component that morphs between a sun and moon icon.
 */
const ThemeToggle = ({
  size = 45,
  sunColor = "#34495e",
  ...
  // Register the GSAP plugin once the component mounts.
  useLayoutEffect(() => {
    gsap.registerPlugin(MorphSVGPlugin);
  }, []);
  ...
  const handleToggle = () => {
    ...
    // Toggle dark class on the root element for Tailwind CSS
    document.documentElement.classList.toggle("dark", nextIsDarkMode);

    // Animate the SVG path shape
    gsap.to(iconPathRef.current, {
      morphSVG: nextIsDarkMode ? moonPath : sunPath,
      duration: 0.5,
      ease: "power3.inOut",
    });
  };
  ...
  return (
    ...
    >
      ...
      >
        <path
          ref={iconPathRef}
          d={sunPath} // Initial shape is the sun
